-- PATCH MIGRATION: finalize backend with submissions and credit locking, fix FK
create extension if not exists pgcrypto with schema public;

-- Ensure enums
DO $$ BEGIN CREATE TYPE public.project_type AS ENUM ('Website','App','Service Flow','Other'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE public.test_status AS ENUM ('active','closed'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE public.feedback_status AS ENUM ('submitted','approved','rejected'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE public.tx_direction AS ENUM ('debit','credit'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE public.tx_reason AS ENUM ('post_test','feedback_approved','purchase','refund','admin_adjust'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- Add required columns
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS credits_locked integer NOT NULL DEFAULT 0;
...
-- Repurpose credit_transactions.feedback_id -> submissions.id
DO $$ BEGIN
  ALTER TABLE public.credit_transactions DROP CONSTRAINT credit_transactions_feedback_id_fkey;
EXCEPTION WHEN undefined_object THEN NULL; END $$;
DO $$ BEGIN
  ALTER TABLE public.credit_transactions
    ADD CONSTRAINT credit_transactions_feedback_id_fkey
    FOREIGN KEY (feedback_id) REFERENCES public.submissions(id) ON DELETE SET NULL;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- Now remove legacy feedbacks safely
  ...
  FOR INSERT WITH CHECK (
    ...
    EXISTS (
      ...
      WHERE tr.id = submissions.test_id
        ...
        AND tr.locked_remaining >= tr.reward
    )
  );
...
CREATE TRIGGER set_test_requests_updated_at BEFORE UPDATE ON public.test_requests FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
DROP TRIGGER IF EXISTS set_profiles_updated_at ON public.profiles;
CREATE TRIGGER set_profiles_updated_at BEFORE UPDATE ON public.profiles FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

-- Lock credits on posting
CREATE OR REPLACE FUNCTION public.before_insert_test_requests()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE needed_total integer; current_balance integer; BEGIN
  ...
  IF current_balance < needed_total THEN RAISE EXCEPTION 'Not enough credits to post this test (need %, have %)', needed_total, current_balance; END IF;
  UPDATE public.profiles SET credits_balance = credits_balance - needed_total, credits_locked = credits_locked + needed_total WHERE id = NEW.owner_id;
...
CREATE TRIGGER trg_before_insert_test_requests BEFORE INSERT ON public.test_requests FOR EACH ROW EXECUTE FUNCTION public.before_insert_test_requests();

-- Transfers on approval/rejection
CREATE OR REPLACE FUNCTION public.after_update_submissions_transfer()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _reward integer; _owner uuid; BEGIN
...
CREATE TRIGGER trg_after_update_submissions_transfer AFTER UPDATE OF status ON public.submissions FOR EACH ROW EXECUTE FUNCTION public.after_update_submissions_transfer();

-- Refund on close/delete
CREATE OR REPLACE FUNCTION public.after_update_test_requests_refund_on_close()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _owner uuid; _remaining integer; BEGIN
...
CREATE TRIGGER trg_after_update_test_requests_refund_on_close AFTER UPDATE OF status ON public.test_requests FOR EACH ROW EXECUTE FUNCTION public.after_update_test_requests_refund_on_close();
...
CREATE TRIGGER trg_before_delete_test_requests_refund BEFORE DELETE ON public.test_requests FOR EACH ROW EXECUTE FUNCTION public.before_delete_test_requests_refund();
...
BEGIN INSERT INTO public.notifications(user_id, type, payload) VALUES (_user_id, _type, coalesce(_payload,'{}'::jsonb)); END; $$;
CREATE OR REPLACE FUNCTION public.after_insert_submission_notify()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _owner uuid; BEGIN SELECT owner_id INTO _owner FROM public.test_requests WHERE id = NEW.test_id; PERFORM public.notify(_owner, 'new_submission', jsonb_build_object('test_id', NEW.test_id, 'submission_id', NEW.id)); RETURN NEW; END; $$;
DROP TRIGGER IF EXISTS trg_after_insert_submission_notify ON public.submissions;
CREATE TRIGGER trg_after_insert_submission_notify AFTER INSERT ON public.submissions FOR EACH ROW EXECUTE FUNCTION public.after_insert_submission_notify();
CREATE OR REPLACE FUNCTION public.after_update_submission_notify()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN IF NEW.status <> OLD.status THEN PERFORM public.notify(NEW.tester_id, 'submission_' || NEW.status, jsonb_build_object('test_id', NEW.test_id, 'submission_id', NEW.id)); END IF; RETURN NEW; END; $$;
DROP TRIGGER IF EXISTS trg_after_update_submission_notify ON public.submissions;
CREATE TRIGGER trg_after_update_submission_notify AFTER UPDATE OF status ON public.submissions FOR EACH ROW EXECUTE FUNCTION public.after_update_submission_notify();

-- Leaderboard view
CREATE OR REPLACE VIEW public.leaderboard_weekly AS
SELECT ct.user_id AS tester_id, sum(ct.amount)::int AS credits_earned, min(ct.created_at) AS first_earn, max(ct.created_at) AS last_earn
FROM public.credit_transactions ct
WHERE ct.direction = 'credit'::public.tx_direction AND ct.reason = 'feedback_approved'::public.tx_reason AND ct.created_at >= now() - interval '7 days'
GROUP BY ct.user_id ORDER BY credits_earned DESC;
...
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE cnt int; BEGIN SELECT count(*) INTO cnt FROM public.test_requests WHERE owner_id = NEW.owner_id; IF cnt >= 10 THEN INSERT INTO public.user_badges(user_id, badge_id) VALUES (NEW.owner_id, 'ten_tests_posted') ON CONFLICT (user_id, badge_id) DO NOTHING; END IF; RETURN NEW; END; $$;
DROP TRIGGER IF EXISTS trg_after_insert_test_requests_award_badge ON public.test_requests;
CREATE TRIGGER trg_after_insert_test_requests_award_badge AFTER INSERT ON public.test_requests FOR EACH ROW EXECUTE FUNCTION public.after_insert_test_requests_award_badge();
CREATE OR REPLACE FUNCTION public.after_update_submission_award_badge()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE cnt int; BEGIN IF NEW.status = 'approved' AND OLD.status IS DISTINCT FROM 'approved' THEN SELECT count(*) INTO cnt FROM public.submissions WHERE tester_id = NEW.tester_id AND status = 'approved'; IF cnt >= 50 THEN INSERT INTO public.user_badges(user_id, badge_id) VALUES (NEW.tester_id, 'fifty_approvals_received') ON CONFLICT (user_id, badge_id) DO NOTHING; END IF; END IF; RETURN NEW; END; $$;
DROP TRIGGER IF EXISTS trg_after_update_submission_award_badge ON public.submissions;
CREATE TRIGGER trg_after_update_submission_award_badge AFTER UPDATE OF status ON public.submissions FOR EACH ROW EXECUTE FUNCTION public.after_update_submission_award_badge();
...
EXCEPTION WHEN undefined_object THEN NULL; END; END $$;
